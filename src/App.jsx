...
function App() {
  ...
  const handleDecoded = () => {
    let storageData =
      userState?.token || JSON.parse(localStorage.getItem("customer"))?.token;
    let decoded = {};
    let refreshToken = "";
    ...
    if (storageData) {
      ...
      refreshToken =
        ...
        JSON.parse(localStorage.getItem("customer"))?.refreshToken;
  ...
  axios.interceptors.request.use(
    async (config) => {
      ...
      const { decoded, storageData, decodedRefreshToken, refreshToken } =
        handleDecoded();
      ...
      if (decoded?.exp < currentTime.getTime() / 1000) {
        if (decodedRefreshToken?.exp > currentTime.getTime() / 1000) {
          ...
          toast.warning(
            "Your login session has expired, please log in again to continue using!"
          );
          localStorage.clear();
